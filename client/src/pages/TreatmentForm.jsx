...
const TreatmentForm = ({ existing, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (existing) {
            const regimen = commonRegimens.includes(existing.regimen) ? existing.regimen : "Other";
            setFormData({
                patient: existing.patient?._id || "",
                diagnosis: existing.diagnosis?._id || "",
                regimen,
                otherRegimen: regimen === "Other" ? existing.regimen || "" : "",
    ...
    const handleSubmit = async (e) => {
        ...
        if (!currentUserId) {
            toast.error("User session expired or Doctor ID missing. Please log in again.");
            return;
        }

        // Basic validation
        if (!formData.patient || !computeRegimenValue() || !formData.startDate) {
        ...
        let payload = {
            ...
            appointmentDate: new Date(formData.startDate).toISOString(),
        ...
        try {
            if (existing) {
                ...
                dispatch(fetchTreatments());  // Fetch treatments after creation
            }
            onClose();
        } catch (err) {
